...
A: `.bruin.yml` could be at a parent folder but `pipeline.yml` and `assets/` must be in the pipeline folder
...
A: using the --var flag and using '' inject the value as a literal 
---
...
A: In Bruin, the `+` suffix on an asset name selects that asset plus all its downstream dependencies. The `--select` flag is used to specify which assets to run.
---
